...
export function MissionCard({ title, description, icon: Icon, onClick, delay = 0, testId }: MissionCardProps) {
  return (
    ...
    >
      <Card
        onClick={onClick}
        className="group cursor-pointer h-full min-h-[220px] bg-card border-2 border-transparent hover:border-primary/40 transition-all duration-300 shadow-md hover:shadow-xl"
        data-testid={testId}
      >
        <CardContent className="flex flex-col items-center justify-center h-full p-6 sm:p-8 text-center gap-4">
          <motion.div
